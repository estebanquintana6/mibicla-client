...
const EditEvent = ({ event }) => {
    ...
    const [startLat, setStartLat] = useState(event.startLat);
    const [startLng, setStartLng] = useState(event.startLng);
    ...
    const options = ["Entusiasta", "Principiante", "Intermedio", "Avanzado"];
    ...
    const handleSubmit = async (e) => {
        ...
        const { status } = await axios.post("/events/update", formData, {
            headers: {
        ...
        if (status === 200) {
            swal("Éxito", "Evento actualizado!", "success")
                .then(() => {
            ...
            <div className="mt-6">
                <form ref={formRef}>
                    ...
                    <div className="w-full flex xs:flex-col md:flex-row flex-wrap mt-4">
                        ...
                        <div className="mb-4 xs:w-full md:w-1/3 md:px-4">
                            ...
                            <input
                                ...
                                defaultValue={new Date(event.date).toISOString().substring(0, 10)}
                                ref={dateRef}
                            />
                        </div>
                        <div className="mb-4 xs:w-full md:w-1/3 md:px-4">
                        ...
                        <div className="mb-6 xs:w-2/4 md:w-2/4 md:px-4 xs:pr-2">
                            ...
                            <select
                                ...
                                {options.map((option) =>
                                    ...
                                    <option value={option} selected={event.difficulty == option}>{option}</option>
                                )}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="w-full flex overflow-y-auto flex-wrap mt-4">
                        <div className="mb-4 xs:w-full md:w-2/4 md:px-4">
                            <img className="xs:my-auto md:my-0 xs:h-32 md:h-64 xs:w-32 md:w-64 object-cover" src={getFile(event.posterUrl)} alt="" />
                    ...
                    {tags.map((tag, index) =>
                        <span
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ml-3 hover:text-gray-300" viewBox="0 0 20 20"
                                fill="currentColor">
                                <path fillRule="evenodd" onClick={removeTag.bind(this, tag)}
